...
def fetch_data_files(data_frame, data_fold, im_suffixe, target_suffixe):
    ...
    for s, c in zip(data_frame.subject.values, data_frame.contrast_foldname.values):
        im_fname = data_fold + s + '/' + c + '/' + s + '_'+ c + im_suffixe + '.nii.gz'
        gt_fname = data_fold + s + '/' + c + '/' + s + '_'+c + target_suffixe + '.nii.gz'
...
def train_model(model, path2save, model_name, training_generator, validation_generator, steps_per_epoch, validation_steps, n_epochs, learning_rate_drop=None, learning_rate_patience=50,batch = 1):
    '''
    Train a Keras model.
    ...
    '''
    return model.fit(training_generator,
                        verbose = 2,
