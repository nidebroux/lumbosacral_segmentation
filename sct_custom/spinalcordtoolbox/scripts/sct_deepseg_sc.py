...
def get_parser():
    ...
    optional.add_agument(
        '-custom',
        metavar=Metavar.int,
        type=int,
        help='Indicate if deepseg needs to use the fine-tuned model',
        choices=[0, 1],
        default=0)
...
def main(argv=None):
    ...
    im_seg, im_image_RPI_upsamp, im_seg_RPI_upsamp = \
        deep_segmentation_spinalcord(im_image.copy(), contrast_type, ctr_algo=ctr_algo,
                                     ...
                                     threshold_seg=threshold, remove_temp_files=remove_temp_files, custom=custom,  verbose=verbose)
